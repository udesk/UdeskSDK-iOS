...
@property (nonatomic, copy) void (^handler)(UdeskAlertAction *action);
- (void)performAction;
@end

@implementation UdeskAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(UDAlertActionStyle)style handler:(void (^)(UdeskAlertAction *action))handler {
    return [[self alloc] initWithTitle:title style:style handler:handler];
}

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(UdeskAlertAction *action))handler {
    return [[self alloc] initWithTitle:title style:UDAlertActionStyleDefault handler:handler];
}

- (instancetype)initWithTitle:(NSString *)title style:(UDAlertActionStyle)style handler:(void (^)(UdeskAlertAction *action))handler {
    if ((self = [super init])) {
        _title = [title copy];
        _style = style;
...
@interface UdeskAlertController () <UIActionSheetDelegate, UIAlertViewDelegate> {
    struct {
...
// Universal
@property (nonatomic, weak) UdeskAlertAction *executedAlertAction;
...
- (void)addAction:(UdeskAlertAction *)action {
    NSAssert([action isKindOfClass:UdeskAlertAction.class], @"Must be of type PSTAlertAction");
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (completion) {
            [self addDidDismissBlock:^(UdeskAlertAction *action) { completion(); }];
        }
        if (self.preferredStyle == UDAlertControllerStyleActionSheet) {
...
- (UdeskAlertAction *)actionForButtonIndex:(NSInteger)index {
    return index >= 0 ? self.actions[index] : nil;
}

- (void)performBlocks:(NSString *)blocksStorageName withAction:(UdeskAlertAction *)alertAction {
    // Load variable and nil out.
    NSArray *blocks = [self valueForKey:blocksStorageName];
    [self setValue:nil forKey:blocksStorageName];

    for (void (^block)(UdeskAlertAction *action) in blocks) {
        block(alertAction);
    }
}

- (void)viewWillDismissWithButtonIndex:(NSInteger)buttonIndex {
    UdeskAlertAction *action = [self actionForButtonIndex:buttonIndex];
...
@implementation UdeskAlertController (Convenience)
