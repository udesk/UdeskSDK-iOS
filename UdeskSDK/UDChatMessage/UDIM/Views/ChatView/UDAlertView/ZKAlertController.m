...
@interface ZKAlertAction ()

@property (nonatomic, copy) void(^actionHandler)(ZKAlertAction *action);

@end

@implementation ZKAlertAction

+ (instancetype)actionWithTitle:(nullable NSString *)title style:(ZKAlertActionStyle)style handler:(void (^ __nullable)(ZKAlertAction *))handler
{
    ZKAlertAction *instance = [ZKAlertAction new];
...
{
    ZKAlertController *instance = [ZKAlertController new];
...
- (void)adjustIndexesforActions
{
    ...
    if (self.mutableActions.count == 2) {
        [self.mutableActions enumerateObjectsUsingBlock:^(ZKAlertAction * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.style == ZKAlertActionStyleCancel) {
...
- (void)creatAllButtons
{
    for (int i = 0; i < self.actions.count; i++) {
        ...
        switch (self.actions[i].style) {
            case ZKAlertActionStyleDefault:
            ...
            case ZKAlertActionStyleDestructive:
...
- (void)configTextScrollViewFrame
{
    ...
    if (textHeight + menuHeight <= _maxHeight) {
        if (textHeight > 90.0) {
            self.textScrollView.frame = CGRectMake(0, 0, _contentViewWidth, 90.0);
        }else{
...
- (void)buttonDidClick:(UIButton *)sender
{
    ...
    ZKAlertAction *action = self.actions[sender.tag - 10];
...
{
    if (action.style == ZKAlertActionStyleCancel) _count ++;
...
- (UILabel *)messageLbel
{
    if (!_messageLbel) {
        ...
        _messageLbel.font = [UIFont boldSystemFontOfSize:17];
