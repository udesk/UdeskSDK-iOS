...
@property (nonatomic, strong) UdeskLocationResultViewModel *resultViewModel;
...
- (void)viewDidLoad {
    ...
    if (!_hasSend) {
        ...
        _mapView.ud_height = UD_SCREEN_HEIGHT-_customNav.ud_bottom;
...
- (void)setupCustomNav {
    ...
    @udWeakify(self)
    _customNav.closeViewController = ^(){
        @udStrongify(self)
    ...
    _customNav.rightButtonHandle = ^{
        @udStrongify(self)
        [self.viewModel getMapSnapshotWithRegion:self.mapView.region mapViewSize:self.mapView.ud_size completion:^(UIImage *snapshot) {
...
- (void)setupMapView {
    ...
    _mapView = [[MKMapView alloc] initWithFrame:CGRectMake(0, self.hasSend?_customNav.ud_bottom:_searchBar.ud_bottom, UD_SCREEN_WIDTH, 200)];
...
- (void)setupNearbayTableView {
    ...
    _nearbyTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, _mapView.ud_bottom, UD_SCREEN_WIDTH, UD_SCREEN_HEIGHT-_mapView.ud_bottom) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UdeskNearbyModel *model = [self.viewModel.nearbyArray objectAtIndexCheck:indexPath.row];
...
- (UdeskLocationResultViewModel *)resultViewModel {

    if (!_resultViewModel) {
        @udWeakify(self);
        _resultViewModel = [[UdeskLocationResultViewModel alloc] init];
...
- (UITableView *)searchResultTableView {
    ...
    if (!_searchResultTableView) {
        _searchResultTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, _searchBar.ud_bottom, UD_SCREEN_WIDTH, UD_SCREEN_HEIGHT-_searchBar.ud_bottom) style:UITableViewStylePlain];
...
- (void)dealloc {
    NSLog(@"UdeskLocationViewController释放了");
}
