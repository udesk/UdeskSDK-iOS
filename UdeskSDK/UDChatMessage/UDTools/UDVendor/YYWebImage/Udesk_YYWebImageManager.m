...
@interface Udesk_YYWebImageApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end
@implementation Udesk_YYWebImageApplicationNetworkIndicatorInfo
@end

@implementation Udesk_YYWebImageManager

+ (instancetype)sharedManager {
    static Udesk_YYWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Udesk_YYImageCache *cache = [Udesk_YYImageCache sharedCache];
...
- (instancetype)initWithCache:(Udesk_YYImageCache *)cache queue:(NSOperationQueue *)queue{
    self = [super init];
    if (!self) return nil;
    _cache = cache;
    _queue = queue;
    _timeout = 15.0;
    if (Udesk_YYImageWebPAvailable()) {
...
+ (Udesk_YYWebImageApplicationNetworkIndicatorInfo *)_networkIndicatorInfo {
    return objc_getAssociatedObject(self, @selector(_networkIndicatorInfo));
}

+ (void)_setNetworkIndicatorInfo:(Udesk_YYWebImageApplicationNetworkIndicatorInfo *)info {
