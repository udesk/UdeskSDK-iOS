...
+ (void)showWithMsg:(NSString *)message {
    ...
    UdeskAlertController *agentAlert = [UdeskAlertController alertControllerWithTitle:nil attributedMessage:[[NSAttributedString alloc] initWithString:message] preferredStyle:UdeskAlertControllerStyleAlert];
    [agentAlert addAction:[UdeskAlertAction actionWithTitle:getUDLocalizedString(@"udesk_close") style:UdeskAlertActionStyleDefault handler:nil]];
...
+ (void)showWithTitle:(NSString *)title message:(NSString *)message handler:(void(^)(void))handler {
    ...
    UdeskAlertController *blacklisted = [UdeskAlertController alertControllerWithTitle:title attributedMessage:[[NSAttributedString alloc] initWithString:message] preferredStyle:UdeskAlertControllerStyleAlert];
    
    [blacklisted addAction:[UdeskAlertAction actionWithTitle:getUDLocalizedString(@"udesk_sure") style:UdeskAlertActionStyleDefault handler:^(UdeskAlertAction * _Nonnull action) {
        if (handler) {
            handler();
        }
    }]];
    
    [blacklisted addAction:[UdeskAlertAction actionWithTitle:getUDLocalizedString(@"udesk_close") style:UdeskAlertActionStyleDefault handler:nil]];
...
+ (void)showBlacklisted:(NSString *)message handler:(void(^)(void))handler {
    ...
    UdeskAlertController *blacklisted = [UdeskAlertController alertControllerWithTitle:nil attributedMessage:[[NSAttributedString alloc] initWithString:message] preferredStyle:UdeskAlertControllerStyleAlert];

    [blacklisted addAction:[UdeskAlertAction actionWithTitle:getUDLocalizedString(@"udesk_close") style:UdeskAlertActionStyleDefault handler:^(UdeskAlertAction * _Nonnull action) {
        if (handler) {
            handler();
        }
    }]];
...
+ (void)showWithAgentCode:(NSInteger)agentCode message:(NSString *)message enableFeedback:(BOOL)enableFeedback leaveMsgHandler:(void(^)(void))leaveMsgHandler {
    ...
    if (agentCode == 2002) {
        ...
        //正在连接提示
        [self showWithMsg:getUDLocalizedString(@"udesk_connecting")];
    }
}
...
+ (void)showAgentOfflineWithAttributedMessage:(NSAttributedString *)attributedMessage enableFeedback:(BOOL)enableFeedback leaveMsgHandler:(void(^)(void))leaveMsgHandler {
    ...
    UdeskAlertController *notOnlineAlert = [UdeskAlertController alertControllerWithTitle:title attributedMessage:attributedMessage preferredStyle:UdeskAlertControllerStyleAlert];
    [notOnlineAlert addAction:[UdeskAlertAction actionWithTitle:cancelButtonTitle style:UdeskAlertActionStyleDefault handler:nil]];
    
    if (enableFeedback) {
        [notOnlineAlert addAction:[UdeskAlertAction actionWithTitle:ticketButtonTitle style:UdeskAlertActionStyleDefault handler:^(UdeskAlertAction * _Nonnull action) {
            if (leaveMsgHandler) {
                leaveMsgHandler();
            }
        }]];
...
+ (void)showQueueWithMessage:(NSString *)message enableFeedback:(BOOL)enableFeedback leaveMsgHandler:(void(^)(void))leaveMsgHandler {
    ...
    UdeskAlertController *queueAlert = [UdeskAlertController alertControllerWithTitle:@"" attributedMessage:[[NSAttributedString alloc] initWithString:message] preferredStyle:UdeskAlertControllerStyleAlert];
    
    [queueAlert addAction:[UdeskAlertAction actionWithTitle:getUDLocalizedString(@"udesk_close") style:UdeskAlertActionStyleDefault handler:nil]];
    
    if (enableFeedback) {
        [queueAlert addAction:[UdeskAlertAction actionWithTitle:title style:UdeskAlertActionStyleDefault handler:^(UdeskAlertAction * _Nonnull action) {
            if (leaveMsgHandler) {
                leaveMsgHandler();
            }
        }]];
