...
@implementation Udesk_YYDiskCache {
    Udesk_YYKVStorage *_kv;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    Udesk_YYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [Udesk_YYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [Udesk_YYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
...
- (NSString *)filePathForkey:(NSString *)key
{
    if (!key) return nil;
    Lock();
    Udesk_YYKVStorageItem *item = [_kv getItemInfoForKey:key];
    Unlock();
