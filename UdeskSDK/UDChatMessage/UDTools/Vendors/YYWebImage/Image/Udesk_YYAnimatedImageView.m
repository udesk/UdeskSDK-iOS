 ...
 */
@interface Udesk_YYImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation Udesk_YYImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[Udesk_YYImageWeakProxy alloc] initWithTarget:target];
}
...
- (void)main {
    __strong Udesk_YYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation Udesk_YYAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[Udesk_YYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)step:(CADisplayLink *)link {
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        Udesk_YYAnimatedImageViewFetchOperation *operation = [Udesk_YYAnimatedImageViewFetchOperation new];
        operation.view = self;
